...
| [0028](0028-shared-cockpit-ui.md) | Paylaşılabilir cockpit UI (4A-2) | **Açık barrel (`web/src/cockpit.ts`), `web/` içinde; fiziksel workspace paketi ERTELENDİ** (`editor/` 4B'de gerçek ihtiyacı tanımlayınca — spekülatif genelleme + CI/codegen churn'ünden kaçın). Transport injection bileşen ağacının yukarısına taşındı (`eventTransport` threaded); auth=transport'un işi, readiness token'dan ayrıştı (`enabled` default=`token.length>0`, davranış korunur) → fork token'sız mount eder. Sınır eslint ile kilitli (paylaşılan UI ⊥ auth/session) | ✅ Uygulandı (4A-2) |
| [0029](0029-webview-shares-web-src.md) | Webview paylaşım mekanizması (4B-3) | **Cross-dir source alias** (kullanıcı kararı): editor webview build'i (esbuild + React) `web/src/cockpit.ts`'i kaynak olarak path-alias ile import eder; `web/` dokunulmaz; bileşenler tek yerde (drift yok); workspace paketi (ADR-0028) ertelenmeye devam. Doğrulama: webview typecheck (alias çözülür, FleetDashboard props bridge transport'larıyla tip-uyumlu) + webview build (cross-dir bundling kanıtı); canlı render opt-in. Editör kendi React'ini getirir (tsc paths→@types + esbuild nodePaths; editor CI job'unda web/node_modules yok) | ✅ Uygulandı (4B-3, CI yeşil) |
| [0030](0030-task-diff-source.md) | Editör task-diff kaynağı (4C-0) | **conductor-emitted bounded `KindDiff` event** (orchestrator kararı): gateway repo görmüyor; diff host-git-local; `KindDiff` rezerve ama emit edilmiyor. conductor (frozen DEĞİL; lifecycle event'leri zaten emit eder) review/merge'de task-branch diff'ini hesaplayıp **bounded** KindDiff (diff-stat + capped patch + truncated bayrağı; PG NOTIFY ~8KB sınırı) emit eder → mevcut bus→gateway→köprü→webview pipe'ı reuse, yeni endpoint YOK, engine.go dokunulmaz. 4C-2 komut + 4C-3 bildirim bu karardan bağımsız | ✅ Karar (4C-0); impl 4C-1 |
| [0031](0031-fork-overlay-build.md) | Fork paketleme stratejisi (4D-0) | **VSCodium-tarzı overlay build** (orchestrator kararı): `everva/conductor-editor` ince orkestrasyon reposu vscode kaynağını VENDOR'LAMAZ; build-time'da pinned Code-OSS (tag 1.122.1 / Node 22) clone + küçük `patches/` (product.json rebrand) + built-in extension inject (vsce; kaynak `editor/`'de kalır) + gulp build (macOS arm64 önce). Hard source-fork DEĞİL → upstream-merge ucuz; ADR-0027 "rebrand HAFİF, derin müdahale yok" ile birebir | ✅ Karar (4D-0); impl 4D-0.. |
