...
- **N-6 (gerçek claude -p e2e):** 🔄 Agent. Mevcut: engine UNIT testleri (malformed/timeout/auth-wall/verdict-parse) zaten var. Boşluk: (A) tick-seviye full-pipeline failure-mode e2e (stub develop-cmd ile blocked vs merge), (B) GERÇEK claude 2.1.181 -p tek atış (opt-in CP_REAL_CLAUDE=1, throwaway repo, time-boxed) → gerçek çıktı parser'a uyuyor mu doğrula. Gate gerçek-claude'a bağlı DEĞİL.
