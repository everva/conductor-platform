...
- **N-7 (Intake):** 🔄 Agent. internal/intake — ADR-0012 senaryo şeması (id/lane/tier/deps/acceptance/holdout) + validation + Distiller seam (CommandEngine/claude -p, çıktı deterministik parse) + file-intake→StateStore persist. Deterministik testler (stub distiller). Gate yeşil kalır.
