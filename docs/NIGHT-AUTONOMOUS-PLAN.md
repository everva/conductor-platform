...
- **N-11 (Heartbeat + harici stall-alert):** 🔄 Agent. ADR-0016 (ürün tarafı, Go). Daemon her tick heartbeat yazar (ts+state); bağımsız stall-detector (conductor heartbeat-check subcmd / tool) bayat heartbeat'i okur→alert (pluggable Notifier, default log+exit-code; secret yok). Deterministik testler (clock inject).
