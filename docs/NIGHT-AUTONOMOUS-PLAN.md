...
- **N-9 (Events):** 🔄 Agent. ADR-0011. ÖNCE fonksiyonel çekirdek: event şeması + phase/kind taksonomi + intervention-needed + EventBus (in-memory + Postgres LISTEN/NOTIFY, docker ile test, gate DB-free skip). TS-codegen ikincil (golden-test minimal emitter veya defer+rapor). Stall riskine karşı dar scope.
