...
- **P3-4: conductorctl paylaşılan store** (ŞİMDİ 🔄) — conductorctl `-dsn`→PostgresStore (yoksa in-memory). Daemon ile AYNI PG → onboard/intake/status/pause/resume gerçekten daemon'ı sürer. (Şu an in-memory: her CLI çağrısı boş process, kalıcı değil.)
- **P3-3: Control reverse-channel** (pause/resume/abort, engine.Command frozen tipi, ADR-0011§4) — N-9'da ertelendi. P3-4'ten sonra.
- **P3-5 (sonra): conductorctl rich-intake delege** — fixture'ları zenginleştir → internal/intake'e delege (N-7 follow-up).
