...
- **İş A — Canlı `claude -p` yeşil-yol tick** ✅ PASSED (bağımsız doğrulandı, throwaway temizlendi). Gerçek claude `Add=a+b` yazdı→commit→verdict pass→bağımsız gate→squash-merge `[task:CALC-1]`→done. NEGATİF: performer pass dedi ama gate fail→blocked, merge yok (canlı anti-sahte-yeşil). Task stdin envelope ile iletiliyor (ADR-0014); claude prose+son-satır-JSON verdict.
...
- **İş A.1 — Holdout'u daemon'a bağla** (🔄 ŞİMDİ, kullanıcı onayı: B'den önce). Filesystem-backed HoldoutStore (`store://`→repo-dışı kök dizin; pg/private sonra) + daemon `-holdout-store`/`-holdout-cmd` flag + conductor'ın gerçek `Scenario.HoldoutRef`'i Verify'a geçirmesi. Deterministik test: görünür-gate-geçer/holdout-fail→blocked. Backward-compat: flag yok=noop. ADDITIVE (engine untouched).
