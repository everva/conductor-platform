...
- **N-8 (Scaffolder):** 🔄 Agent. internal/scaffolder — stack-detect (go.mod/package.json/pyproject...) + per-stack profil (build/test/lint reçete defaultları) + .conductor/ taslak üretimi + readiness-gate (testsiz repo→"önce kalite altyapısı"). Fixture-repo deterministik testler. ADR-0009.
